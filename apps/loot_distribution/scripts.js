...
function calculate_distribution() {
  ...
  for (hash in module_ids) {
    ...
    const quantity = parseInt(document.getElementById(module_id.quantity).value)
    const cost = parseFloat(document.getElementById(module_id.cost).value)
  ...
  for (let i = 0; i < modules.length; i++) {
    ...
    for (let k = 0; k < quantity; k++) {
      loot.sort((member1, member2) => member1.loot_value > member2.loot_value ? 1 : -1)
